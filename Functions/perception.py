...
def move():
    ...
    # 不同颜色木快放置坐标(x, y, z)
    # Positioning coordinates of different color wooden blocks
    coordinate = {
    ...
    while True:
        if __isRunning:
            if first_move and start_pick_up: # 当首次检测到物体时 when an object is first detected              
                ...
                setBuzzer(0.1)               
                result = AK.setPitchRangeMoving((world_X, world_Y - 2, 5), -90, -90, 0) # 不填运行时间参数，自适应运行时间 Do not fill in the running time parameter, adaptive running time
                if result == False:
                    unreachable = True
                else:
                    unreachable = False
                time.sleep(result[2]/1000) # 返回参数的第三项为时间 the third item of the returned parameter is the time
                start_pick_up = False
                first_move = False
                action_finish = True
            elif not first_move and not unreachable: # 不是第一次检测到物体 not the first time an object has been detected
                set_rgb(detect_color)
                if track: # 如果是跟踪阶段 if it is the tracking phase
                ...
                if start_pick_up: #如果物体没有移动一段时间，开始夹取 if the object has not moved for a while, start gripping
                    ...
                    if not __isRunning:
                        continue
                    AK.setPitchRangeMoving((world_X, world_Y, 2), -90, -90, 0, 1000)  # 降低高度 lower the altitude
                    time.sleep(2)
                    ...
                    if not __isRunning:
                        continue
                    # 对不同颜色方块进行分类放置 sort and place blocks of different colors
...
def run(img):
    ...
    frame_lab = cv2.cvtColor(frame_gb, cv2.COLOR_BGR2LAB)  # 将图像转换到LAB空间 convert image to LAB space (lightness, magenta/green, yellow/blue)
    ...
    if not start_pick_up:
        for i in color_range:
            if i in __target_color:
                detect_color = i
                # Mask pixels that fall within a particular range
                frame_mask = cv2.inRange(frame_lab, color_range[detect_color][0], color_range[detect_color][1])  # 对原图像和掩模进行位运算 bitwise operations on the original image and mask
        ...
        if area_max > 2500:  # 有找到最大面积 have found the largest area
            # put a rectangle around contour
            rect = cv2.minAreaRect(areaMaxContour)
            box = np.int0(cv2.boxPoints(rect))

            roi = getROI(box) #获取roi区域 get roi region
            get_roi = True

            # get the middle of the box
            img_centerx, img_centery = getCenter(rect, roi, size, square_length)  # 获取木块中心坐标 get the coordinates of the center of the block
